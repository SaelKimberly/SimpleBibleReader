namespace Sword.reader
{
    ...
    public class BibleTextReader 
    {
        ...
        {
            return new int[][][] { new int[][] { otbks, otcps }, new int[][] { ntbks, ntcps } };
        }

        public BibleTextReader(string path, string bible_name)
        {
            filepath = path;
        ...
        {
            string vss_ot_path = filepath + "ot.vss";
            string vss_nt_path = filepath + "nt.vss";
            string raw_ot_path = filepath + "ot";
            string raw_nt_path = filepath + "nt";
            ...
            else if (!File.Exists(raw_nt_path))
            {
                start_book = 1;
            }
            else if (!File.Exists(raw_ot_path) && !File.Exists(raw_nt_path))
                return "";


            //BinaryReader vss = null;// new BinaryReader(File.OpenRead(vss_path));
            //BinaryReader rawText = null;
            int[][][] offsets = getOffsets();

            int offset = 0, testament = 1;
            int index;

            //vss = new BinaryReader(File.OpenRead(vss_ot_path));
            ...
            int size = 0;
             ...
             StringBuilder t_xml = new StringBuilder();
             StringBuilder t_book = new StringBuilder();
             StringBuilder t_chap = new StringBuilder();
             string versetxt = "";
             t_xml.Append("<XMLBIBLE>\n");
             for (int book_id = start_book; book_id <= end_book; book_id++)
             {
                 if (book_id < 40)
                 {
                     testament = 1;
                     if (!File.Exists(vss_ot_path) || !File.Exists(raw_ot_path))
                         continue;
                     fs_vss = File.OpenRead(vss_ot_path);
                     fs_raw = File.OpenRead(raw_ot_path);
                 }
                 else if (book_id >= 40 && book_id <= 66)
                 {
                     testament = 2;
                     if (!File.Exists(vss_nt_path) || !File.Exists(raw_nt_path))
                         continue;
                     fs_vss = File.OpenRead(vss_nt_path);
                     fs_raw = File.OpenRead(raw_nt_path);
                 }
                 else
                     continue;
                 t_book.Length = 0;
                 ...
                 {
                     t_chap.Length = 0;
                     for (int verse = 1; verse <= FormatUtil.GetVerseCount(book_id, chap_id); verse++)
                     {
                         if(testament==1)
                            offset = offsets[testament - 1][0][book_id];
                         else if (testament == 2)
                             offset = offsets[testament - 1][0][book_id-39];
                         offset = offsets[testament - 1][1][offset + chap_id];
                         index = (offset + verse) * 6;
                         fs_vss.Seek(index, SeekOrigin.Begin);
                         start = getintFromStream(fs_vss);
                         size = getShortIntFromStream(fs_vss);
                         fs_raw.Seek(start, SeekOrigin.Begin);
                         bytes = new byte[size];
                         fs_raw.Read(bytes, 0, size);
                         if (size == 0)
                             continue;
                         if (bytes[0] == 0)
                             continue;
                         versetxt=Encoding.UTF8.GetString(bytes).Trim();
                        if (versetxt != "")
                        {
                            t_chap.Append("<VERS vnumber='");
                            t_chap.Append(verse.ToString());
                            t_chap.Append("'>");
                            switch (type)
                            {
                                case FormatUtil.CONTENT_TYPE.BIBLE:
                                    if (GlobalMemory.getInstance().ParseBible)
                                    {
                                        if (!GlobalMemory.getInstance().parseBibleBkgndCleaner)
                                            versetxt = BibleFormat.postProcessContent(versetxt);
                                        versetxt = FormatUtil.EscapeXML(versetxt);
                                    }
                                    else
                                        versetxt = FormatUtil.EscapeXML(versetxt);
                                    break;
                                case FormatUtil.CONTENT_TYPE.COMMENTARY:
                                    if (GlobalMemory.getInstance().ParseCommentary)
                                        versetxt = CommentaryFormat.postProcessContent(versetxt);
                                    else
                                        versetxt = FormatUtil.EscapeXML(versetxt);
                                    break;
                            }
                            t_chap.Append(versetxt);
                            t_chap.Append("</VERS>\n");
                        }
                     }
                     if (t_chap.Length != 0)
                     {
                         t_book.Append("<CHAPTER cnumber='");
                         t_book.Append(chap_id.ToString());
                         t_book.Append("'>\n");
                         t_book.Append(t_chap.ToString());
                         t_book.Append("\n</CHAPTER>\n");
                     }
                 }
                 if (t_book.Length != 0)
                 {
                     t_xml.Append("<BIBLEBOOK bnumber='");
                     t_xml.Append(book_id.ToString());
                     t_xml.Append("'>\n" );
                     t_xml.Append(t_book.ToString());
                     t_xml.Append("\n</BIBLEBOOK>\n");
                 }
                 if (fs_vss != null)
                     fs_vss.Close();
             ...
             t_xml.Append("</XMLBIBLE>");
            return t_xml.ToString(); 
        }

        protected int getintFromStream(FileStream fs)
        {
            byte[] buf = new byte[4];
            fs.Read(buf, 0, 4);
