...
{    
    ...
    public class CMTXCommentaryFormat : CommentaryFormat
    {
        ...
        public CMTXCommentaryFormat(string file)
            : base(file)
        {
        }
        ...
        {
            if (!filename.EndsWith(".cmtx"))
            {
                filename = filename + ".cmtx";
            }
            if (filename.Equals(""))
            {
                return;
            ...
            using (SQLiteConnection sqlConn = new SQLiteConnection())
            {
                ...
                using (SQLiteCommand sqlCommand = new SQLiteCommand(sqlConn))
                {
                    ...
                    command.Append("CREATE TABLE Books (Book INT, Comments TEXT)");
                ...
                {
                    System.Text.StringBuilder command = new System.Text.StringBuilder();
                    command.Append("CREATE TABLE Chapters (Book INT, Chapter INT, Comments TEXT)");
                    sqlCommand.CommandText = command.ToString();
                    sqlCommand.ExecuteNonQuery();
                    sqlCommand.Dispose();
                }

                SQLiteBulkInsert sbi_book = new SQLiteBulkInsert(sqlConn, "Books");
                SQLiteBulkInsert sbi_chap = new SQLiteBulkInsert(sqlConn, "Chapters");
                ...
                {
                    System.Text.StringBuilder command = new System.Text.StringBuilder();
                    command.Append("CREATE TABLE Verses (Book INT, ChapterBegin INT, ChapterEnd INT, VerseBegin INT, VerseEnd INT, Comments TEXT)");
                ...
                using (SQLiteCommand sqlCommand = new SQLiteCommand(sqlConn))
                {
                    ...
                    System.Text.StringBuilder command = new System.Text.StringBuilder();
                    command.Append("CREATE INDEX BookChapterIndex ON Chapters (Book, Chapter)");
                    sqlCommand.CommandText = command.ToString();
                    sqlCommand.ExecuteNonQuery();
                    sqlCommand.Dispose();
                }
                ...
                {
                    System.Text.StringBuilder command = new System.Text.StringBuilder();
                    command.Append("CREATE INDEX BookChapterVerseIndex ON Verses (Book, ChapterBegin, VerseBegin)");
                    sqlCommand.CommandText = command.ToString();
                    sqlCommand.ExecuteNonQuery();
                    sqlCommand.Dispose();
                }
                ...
                {
                    System.Text.StringBuilder command = new System.Text.StringBuilder();
                    command.Append("CREATE TABLE Details (Description NVARCHAR(255), Abbreviation NVARCHAR(50), Comments TEXT, Version INT)");
                    sqlCommand.CommandText = command.ToString();
                    sqlCommand.ExecuteNonQuery();
                    sqlCommand.Dispose();
                }
                        ...
                        for (int v = 1; v <= FormatUtil.GetVerseCount(b, c); v++)
                        {
                            ...
                            if (vers != null)
                            {
                                ...
                                */
                                ...
                                if (vers != null)
                                    sbi.Insert(new object[] { b.ToString(), c.ToString(), c.ToString(), v.ToString(), v.ToString(), GetRtfUnicodeEscapedString(_verse_text) });
                            }
                        }
                }
                sbi.Flush();
