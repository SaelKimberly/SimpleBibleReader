...
{    
    public class DCTIDictionaryFormat : DictionaryFormat
    {
        ...
        public DCTIDictionaryFormat(string file)
            : base(file)
        {
        }
        ...
        {
            if (!filename.EndsWith(".dcti"))
            {
                filename = filename + ".dcti";
            }
            if (filename.Equals(""))
            {
                return;
            ...
            using (SQLiteConnection sqlConn = new SQLiteConnection())
            {
                ...
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    item = getText(items[i],"");
                    item = item.Replace("\n", @"\par ").Trim();
                    sbi.Insert(new object[] { id, GetHtmlUnicodeEscapedString(item) });
                }
                sbi.Flush();
                sqlConn.Dispose();
                }
            ...
            private string getText(XmlNode child, string dictionary_text)
            {
            if (child.HasChildNodes)
            {
                foreach (XmlNode c in child.ChildNodes)
                {
                    dictionary_text = getText(c, dictionary_text);
                }
            }
            else
            ...
            string GetHtmlUnicodeEscapedString(string s)
            {
