...
namespace Simple_Bible_Reader
{
    ...
    public class SwordSearcherBookFormat : BookFormat
    {
        ...
        public SwordSearcherBookFormat(string file)
            : base(file)
        {
                ...
                try
                {
                    ...
                    using (MemoryStream mstream = new MemoryStream(filebytes))
                    {
                        ...
                        for (int i = 0; i < total_words; i++)
                        {
                            ...
                            _notes_text = Encoding.UTF8.GetString(zLibUncompress(bytes_to_read));
        ...
        private void processLine()
        {
            ...
            if (GlobalMemory.getInstance().ParseBook)
            {
                ...
                bookdoc.InnerText = FormatUtil.EscapeXML(_notes_text);
            }
            else
        ...
        public override void ExportBook(string filename, int filter_idx)
        {
            ...
            for (int i = 0; i < titles.Count; i++)
            {
                ...
                {                    
                    ...
                    writer.WriteLine("$$ " + titles[i].Value);
