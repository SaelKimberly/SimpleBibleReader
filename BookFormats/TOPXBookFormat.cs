...
namespace Simple_Bible_Reader
{
    ...
    public class TOPXBookFormat : BookFormat
    {
        ...
        public TOPXBookFormat(string file)
            : base(file)
        {
        }
        ...
        public override void Load()
        {
            ...
            try
            {
                ...
                if (this.FileName.EndsWith(".refx"))
                {
                ...
                else if (this.FileName.EndsWith(".topx"))
                {
                    mycommand.CommandText = "SELECT Title, Notes FROM topics";
                    is_refx = false;
                    reader = mycommand.ExecuteReader();
        ...
        public override void ExportBook(string filename, int filter_idx)
        {
            ...
            if (!filename.EndsWith(".topx"))
            {
                filename = filename + ".topx";
            }
            ...
            using (SQLiteConnection sqlConn = new SQLiteConnection())
            {
                ...
                command.Append("CREATE TABLE Topics (Title NVARCHAR (100), Notes TEXT)");
                sqlCommand.CommandText = command.ToString();
                sqlCommand.ExecuteNonQuery();
                sqlCommand.Dispose();

                sbi = new SQLiteBulkInsert(sqlConn, "Topics");
                sbi.AddParameter("Title", DbType.String);
                sbi.AddParameter("Notes", DbType.String);
                ...
                for (int i = 0; i < titles.Count; i++)
                {
                    ...
                    if (note != null)
                    {
                        ...
                        _note_text = rtf_header + _note_text.Replace("\n", "\\par") + "\n}";
