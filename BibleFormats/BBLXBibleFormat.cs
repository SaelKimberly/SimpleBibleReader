...
{    
    ...
    public class BBLXBibleFormat : BibleFormat
    {
        ...
        public BBLXBibleFormat(string file)
            : base(file)
        {
        }
        ...
        {
            if (!filename.EndsWith(".bblx"))
            {
                filename = filename + ".bblx";
            }
            if (filename.Equals(""))
            {
                return;
            ...
            using (SQLiteConnection sqlConn = new SQLiteConnection())
            {
                ...
                using (SQLiteCommand sqlCommand = new SQLiteCommand(sqlConn))
                {
                    ...
                    command.Append("INSERT INTO Details VALUES('" + Path.GetFileName(filename) + "','" + Path.GetFileNameWithoutExtension(filename) +
                        "','Exported from Simple Bible Reader (" + GlobalMemory.AUTHOR_WEBSITE + ")',1,'DEFAULT'," + dir.ToString() + ",1,1,"+ apocrypha.ToString() + ",0)");
                    */

                    command.Append("INSERT INTO Details VALUES('" + DESCRIPTION + "','" + ABBREVIATION +
                        "','"+COMMENTS+"',"+mVERSION+",'"+mFONT+"'," + dir.ToString() + ","+mOT+","+mNT+"," + apocrypha.ToString() + ","+mSTRONG+")");
