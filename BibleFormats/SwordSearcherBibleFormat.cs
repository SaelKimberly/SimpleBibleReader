...
{    
    ...
    public class SwordSearcherBibleFormat : BibleFormat
    {
        ...
        public SwordSearcherBibleFormat(string file)
            : base(file)
        {
        }
                    ...
                    using (MemoryStream mstream = new MemoryStream(filebytes))
                    {
                        ...
                        for (int i = 0; i < total_verses; i++)
                        {
                            ...
                            try
                            {
                                ...
                                string uncompressed = Encoding.UTF8.GetString(FormatUtil.eSword_zLibUncompress(bytes_to_read));
                                //remove strongs
                                uncompressed = uncompressed.Replace("\u0002", "").Replace("\u0003", "");
                                uncompressed = processStrongsHex2Dec(uncompressed);
        ...
        private string processStrongsHex2Dec(string verse)
        {
            ...
            {                
                ...
                splitwords = verse.Split(new char[] { '$' });
                sb.Append(splitwords[0]);
                for (int i = 1; i < splitwords.Length; i++)
                {
                    word = splitwords[i];

                    string strong = word.Substring(0, 4);
                    string remaining = word.Substring(4);

                    int istrong = Convert.ToInt16(strong, 16);
                    sb.Append("#");
                    sb.Append(istrong.ToString());
                    sb.Append("#");
                    sb.Append(" ");
                    sb.Append(remaining);
                }
        ...
        {
            filename = Regex.Replace(filename, "\\.ss[56789]bible", "");
